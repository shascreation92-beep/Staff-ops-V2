...
export default async function LiveTeamOperationsMirrorPage({ params }: PageProps) {
  ...
  const [
    ...
    db.account.count({ where: { createdById: targetTL.id, status: "SORTED", issueType: { notIn: ["Marketplace Issue", "Identity Issue", "Suspended", "Code Issue"] }, isArchived: false, ...fbWhere } }),
    ...
    db.account.count({ where: { createdById: targetTL.id, status: "SORTED", issueType: "Code Issue", isArchived: false, ...fbWhere } }),
